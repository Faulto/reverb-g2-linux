...
SensorsFW: 1.9.53
Found presence device: \?\HID#VID_03F0&PID_0580#<redacted-instance-suffix>#{4d1e55b2-f16f-11cf-88cb-001111000030}
...
[... same three "sensors device" lines AND the same presence device line, byte-identical
     instance IDs, including "<redacted-instance-suffix>" — see note below ...]
...
presence device's instance ID suffix, **`<redacted-instance-suffix>`, is byte-identical between shot
2 and shot 3** — i.e. surviving a real USB disconnect/reconnect *in the same port*. If a
future capture in a **different** port changes that `<redacted-instance-fragment>` segment, it's consistent with
...
`<redacted-hub-serial>`, shared identically by both the SuperSpeed and USB2 personas of the same
...
VID_03F0&PID_0580\<redacted-instance-suffix>`:**

```
"ContainerID"="<redacted-container-id>"
```

**This is the parent Cypress hub's own hardware serial** (`<redacted-hub-serial>`,
...
export, the instance-ID suffix (`<redacted-instance-suffix>`) — a hash of the parent's path, which

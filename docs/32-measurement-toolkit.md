...
taken references nothing anyone will ever look for. Two local captures were renamed on
2026-08-19 (T230) to describe their contents rather than their timestamps. They are
not distributed here: one was a copyrighted manual excerpt and the other a raw system
report. Their useful conclusions were transcribed into the documentation instead.
...
* **A date only when the capture is a dated measurement** whose value depends on when it was
  taken — for example `gpu-driver-summary-2026-08-05`. Then `YYYY-MM-DD`, which sorts correctly, and never

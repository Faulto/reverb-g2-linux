...
| `nvidia-bug-report.log.gz` | submitted to NVIDIA, but intentionally not redistributed here because a system report contains dense host metadata |
...
- `nvidia-bug-report.log.gz` — submitted with the forum report; intentionally excluded from this repository's clean release because it contains host metadata

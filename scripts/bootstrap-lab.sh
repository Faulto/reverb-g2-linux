#!/bin/bash
# bootstrap-lab.sh - archived Debian 13 lab reconstruction helper.
#
...
#
# The historical source export is not one linear patch series. The sources/all
# actions therefore fail closed instead of producing an unverified hybrid tree.
...
if [ "$STEP" != sources ] && [ "$STEP" != patch-nv ] && \
   [ ! -f /etc/vr-lab-machine ] && command -v nvidia-smi >/dev/null 2>&1; then
...
if [ "$STEP" != sources ] && [ "$STEP" != patch-nv ]; then
    command -v apt >/dev/null || { echo "This archived lab action is for Debian/Ubuntu." >&2; exit 1; }
fi
...
do_sources() {
	cat >&2 <<-EOF
	The native G2-controller Monado patches are a research archive, not one
	linear series: 0016/0017 supersede a different lineage and 0024 onward was
	developed on another base. Refusing to construct an unverified source tree.

	For the reproducible G2 + Lighthouse/Index profile, use:
	    $REPO_DIR/scripts/setup-index-controllers.sh all

	Researchers continuing the native-controller work should first publish a
	pinned source branch or bundle and then replace this guard with a clean
	series. See patches/monado/README.md.
	EOF
	return 2
}

# Retained as documentary evidence of the original lab procedure. This is not
# dispatched by the public CLI while its patch lineage is unresolved.
do_sources_archived() {
	mkdir -p "$BASE"
	cd "$BASE" || exit 1
...
do_patch_nv() {
	exec "$REPO_DIR/scripts/nvidia-g2-patch-manager.sh" apply
}

do_patch_nv_archived() {
